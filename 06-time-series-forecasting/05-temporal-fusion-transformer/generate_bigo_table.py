"""Generate a Big-O / cost summary table image for the TFT article."""
...
ROWS = [
    ['Training per epoch',
     'O(N · W · d²)',
     'O(B · W · d)',
     'global across all N series; GPU-friendly'],
    ['Full training (E epochs)',
     'O(E · N · W · d²)',
     'O(B · W · d)',
     'E ≈ 50–200 typical'],
    ...
    ['Inference per forecast',
     'O(H · W · d²)',
     'O(W · d)',
     'autoregressive over H horizon steps'],
    ['Quantile loss eval',
     'O(Q)',
     'O(Q)',
     'pinball loss at each of Q quantiles'],
    ['vs. classical (per-series)',
     'fits 1 model',
     '—',
     'classical fits N independent models'],
]

COL_WIDTHS = [5.0, 3.4, 2.6, 6.0]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Temporal Fusion Transformer Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'N = series count, W = window length, H = forecast horizon, '
        'd = hidden dim, B = batch size, E = epochs, Q = quantiles.',
...
out = ('D:/Projects/Medium/algorithms-in-python/'
       ...
       '05-temporal-fusion-transformer/bigo_table.png')
