"""Generate a Big-O / cost summary table image for the Prophet article."""
...
ROWS = [
    ['MAP fit (default)',
     'O(I · n · f)',
     'O(n + f²)',
     'L-BFGS on Stan model; sub-second to seconds'],
    ['Full Bayesian fit (MCMC)',
     'O(S · n · f)',
     'O(S · f)',
     'Stan NUTS; minutes to hours; better uncertainty'],
    ['Forecast h steps',
     'O(h · f)',
     'O(f)',
     'evaluate fitted decomposition at future ds'],
    ['Cross-validation',
     'O(K · MAP fit)',
     'O(n)',
     'K rolling-origin folds for hyperparam tuning'],
    ['Add custom seasonality',
     'O(2 · order)',
     'O(2 · order)',
     'extra Fourier terms; cheap'],
    ['Plot components',
     'O(n)',
     'O(n)',
     'matplotlib panels; free diagnostics'],
]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'n = observations, f = features (trend coeffs + Fourier terms + holidays), '
        'I = optimiser iters, S = MCMC samples, K = CV folds.',
