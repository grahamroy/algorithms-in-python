"""Generate a Big-O / cost summary table image for the State-Space Models article."""
...
ROWS = [
    ['Kalman filter, one step',
     'O(d²)',
     'O(d²)',
     'optimal Bayesian state update'],
    ['Forward filter, full series',
     'O(T · d²)',
     'O(T · d)',
     'one pass over all observations'],
    ['Kalman smoother',
     'O(T · d²)',
     'O(T · d)',
     'backward pass; refines all states'],
    ['MLE fitting',
     'O(I · T · d²)',
     'O(T · d)',
     'optimiser runs filter at each step'],
    ['EM fitting',
     'O(I · T · d²)',
     'O(T · d)',
     'alternating smoother + parameter update'],
    ['Forecast h steps',
     'O(h · d²)',
     'O(d²)',
     'propagate state covariance forward'],
]

COL_WIDTHS = [5.0, 3.4, 2.6, 5.8]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'T = observations, d = state dimension, h = forecast horizon, '
        'I = optimiser iterations.',
...
out = ('D:/Projects/Medium/algorithms-in-python/'
       ...
       '03-state-space-models/bigo_table.png')
