"""Generate a Big-O / cost summary table image for the ETS article."""
...
ROWS = [
    ...
    ['Fit Holt-Winters',
     'O(I · n)',
     'O(m)',
     'three parameters (α, β, γ); m seasons'],
    ['Forecast h steps',
     'O(h)',
     'O(1)',
     'recurrence application; near-instant'],
    ['ETS auto-selection',
     'O(K · I · n)',
     'O(m)',
     'K candidate (E, T, S) configurations'],
    ['TBATS (multi-seasonal)',
     'O(I · n · M)',
     'O(M · m)',
     'M simultaneous seasonal periods'],
]

COL_WIDTHS = [5.0, 3.2, 2.6, 6.0]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Exponential Smoothing Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'n = observations, h = forecast horizon, I = optimiser iterations, '
        'm = seasonal period, M = number of seasonalities, K = candidate models.',
...
out = ('D:/Projects/Medium/algorithms-in-python/'
       ...
       '02-exponential-smoothing/bigo_table.png')
