"""Generate a Big-O / cost summary table image for the ARIMA article."""
...
ROWS = [
    ['Fit ARIMA(p, d, q)',
     'O(I · n · (p + q))',
     'O(n)',
     'maximum likelihood; I = optimiser iters'],
    ['Fit SARIMA(p, d, q)(P, D, Q, s)',
     'O(I · n · (p + q + P + Q))',
     'O(n)',
     'seasonal extension; state-space EM'],
    ['Forecast h steps ahead',
     'O(h · (p + q))',
     'O(p + q)',
     'tiny; recursive linear update'],
    ['Predictive variance',
     'O(h · (p + q)²)',
     'O((p + q)²)',
    ...
    ['auto_arima search',
     'O(P · Q · D · fit)',
     'O(n)',
     'grid over candidate (p, d, q)'],
    ['Refit on new observation',
     'O(fit)',
     'O(n)',
     'online forecasting via re-estimation'],
]

COL_WIDTHS = [5.4, 4.4, 2.6, 5.4]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'n = observations, p/q = AR/MA orders, P/Q = seasonal orders, '
        'h = forecast horizon, I = iterations.',
