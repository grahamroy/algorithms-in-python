"""Generate a Big-O / cost summary table image for the UMAP article."""
...
ROWS = [
    ['Approximate k-NN graph',
     'O(n^1.14)',
     'O(n · k)',
     'PyNNDescent; sub-quadratic in n'],
    ['Fuzzy simplicial complex',
     'O(n · k)',
     'O(n · k)',
     'per-point adaptive bandwidth'],
    ['Spectral init',
     'O(n · k)',
     'O(n · k)',
    ...
    ['SGD optimisation',
     'O(n · k · epochs)',
     'O(n · k)',
     'attractive + negative sampling'],
    ['Predict (transform)',
     'O(m · k)',
     'O(n · k)',
     'm new points, SGD against fixed train embedding'],
    ['Practical limit',
     'n > 10⁶',
     'O(n · k)',
     'genuinely scales; t-SNE breaks at ~10⁵'],
]
...
ax.text(TABLE_W / 2 + 1.0, caption_y,
        'n = data points, k = n_neighbors, m = new points to '
        'transform, epochs = SGD iterations.',
