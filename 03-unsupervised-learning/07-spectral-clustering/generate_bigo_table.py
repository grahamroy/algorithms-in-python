"""Generate a Big-O / cost summary table image for the Spectral Clustering article."""
...
ROWS = [
    ['Dense RBF affinity',
     'O(n² · d)',
     'O(n²)',
     'all pairs; impractical past n ≈ 10⁴'],
    ['k-NN sparse affinity',
     'O(n · k · d)',
     'O(n · k)',
     'with approximate NN; default in sklearn'],
    ['Laplacian construction',
     'O(n²)',
     'O(n²)',
     'or O(n · k) on sparse k-NN graphs'],
    ['Eigendecomposition (full)',
     'O(n³)',
     'O(n²)',
     'only the bottom K eigenvectors needed'],
    ['Eigendecomposition (Lanczos)',
     'O(n² · K)',
     'O(n · K)',
     'sklearn default; sparse-friendly'],
    ['K-Means in eigen-space',
     'O(I · n · K²)',
     'O(n · K)',
     'cheap; K dimensions, well-separated clusters'],
]

COL_WIDTHS = [5.4, 3.6, 2.6, 5.8]
...
ax.text(TABLE_W / 2 + 1.0, caption_y,
        'n = data points, d = features, k = neighbours in k-NN '
        'graph, K = number of clusters, I = K-Means iterations.',
...
out_path = ('D:/Projects/Medium/algorithms-in-python/'
            ...
            '07-spectral-clustering/bigo_table.png')
