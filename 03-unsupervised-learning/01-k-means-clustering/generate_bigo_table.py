"""Generate a Big-O / cost summary table image for the K-Means article."""
...
ROWS = [
    ['Assignment step',
     'O(n · K · d)',
     'O(n)',
     'distance to every centroid, argmin per point'],
    ['Update step',
     'O(n · d)',
     'O(K · d)',
    ...
    ['Full training',
     'O(I · n · K · d)',
     'O(K · d)',
     'I iterations, usually 10–50 to convergence'],
    ['k-means++ init',
     'O(n · K · d)',
     'O(n)',
     'one weighted sample per centroid'],
    ['Predict one example',
     'O(K · d)',
     'O(K · d)',
     'distance to K centroids; very fast'],
    ['Mini-batch K-Means',
     'O(I · b · K · d)',
     'O(K · d)',
     'b = batch size; scales to streaming data'],
]

COL_WIDTHS = [4.6, 3.8, 2.6, 6.0]
...
ax.text(TABLE_W / 2 + 1.0, caption_y,
        'n = data points, K = number of clusters, d = features, '
        'I = iterations to convergence, b = mini-batch size.',
...
out_path = ('D:/Projects/Medium/algorithms-in-python/'
            ...
            '01-k-means-clustering/bigo_table.png')
