"""Generate a Big-O / cost summary table image for the t-SNE article."""
...
ROWS = [
    ['High-d similarities (P)',
     'O(n² · d)',
     'O(n²)',
     'pairwise distances + per-row binary search'],
    ['Naive t-SNE (per iter)',
     'O(n²)',
     'O(n²)',
     'q_ij sums over all pairs'],
    ['Barnes-Hut t-SNE (per iter)',
     'O(n log n)',
     'O(n · k)',
     'quadtree on low-d points; sklearn default'],
    ['FIt-SNE (per iter)',
     'O(n)',
     'O(n)',
     'FFT-based; needed past n ≈ 10⁵'],
    ['Iterations',
     '~1000',
     '—',
     'fixed budget; early-exaggeration first 250'],
    ['Predict new point',
     '—',
     '—',
     'no native support; t-SNE is not parametric'],
]

COL_WIDTHS = [5.6, 3.4, 2.6, 5.8]
...
ax.text(TABLE_W / 2 + 1.0, caption_y,
        'n = data points, d = original features, k = neighbours kept '
        'in fast P approximations.',
