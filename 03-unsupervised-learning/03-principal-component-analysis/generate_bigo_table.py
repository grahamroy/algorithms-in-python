"""Generate a Big-O / cost summary table image for the PCA article."""
...
ROWS = [
    ...
    ['Full SVD (n > d)',
     'O(n · d²)',
     'O(n · d)',
     'standard PCA, exact components'],
    ['Truncated SVD (top k)',
     'O(n · d · k)',
     'O(n · d)',
     'sparse-friendly; LSA on text'],
    ['Randomised SVD',
     'O(n · d · log k)',
     'O(n · d)',
     'sklearn default for large n or d'],
    ['Project new data',
     'O(d · k)',
     'O(d · k)',
     'one matrix multiply against components'],
    ['Reconstruct from projection',
     'O(d · k)',
     'O(n · d)',
     'inverse_transform; recovery is lossy'],
]
...
ax.text(TABLE_W / 2 + 1.0, caption_y,
        'n = samples, d = original features, k = number of '
        'components retained.',
...
out_path = ('D:/Projects/Medium/algorithms-in-python/'
            ...
            '03-principal-component-analysis/bigo_table.png')
