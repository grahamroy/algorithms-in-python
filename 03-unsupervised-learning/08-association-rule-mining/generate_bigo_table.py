"""Generate a Big-O / cost summary table image for the ARM article."""
...
ROWS = [
    ['Pass over transactions',
     'O(N · |C_k| · k)',
     'O(|C_k|)',
     'count support of every candidate itemset'],
    ['Candidate generation',
     'O(|L_{k-1}|² · k)',
     'O(|C_k|)',
     'join + prune; uses Apriori property'],
    ['Total Apriori run',
     'O(K · N · |L|)',
     'O(|L|)',
     'K = max itemset size; |L| total frequent'],
    ['FP-Growth (alternative)',
     'O(N · k · log N)',
     'O(N · k)',
     'no candidate gen; 5–10× faster on dense data'],
    ['Rule generation',
     'O(|L| · 2^k)',
     'O(|rules|)',
     'subsets of each frequent itemset'],
    ['Worst case (dense data)',
     'O(2^|I|)',
     'O(2^|I|)',
     'happens if min_support set too low'],
]

COL_WIDTHS = [4.8, 3.8, 2.8, 6.4]
...
ax.text(TABLE_W / 2 + 1.0, title_y,
        'Association Rule Mining Complexity',
...
ax.text(TABLE_W / 2 + 1.0, caption_y,
        'N = transactions, |I| = number of items, k = current level, '
        'K = max itemset size, |L| = total frequent itemsets.',
...
out_path = ('D:/Projects/Medium/algorithms-in-python/'
            ...
            '08-association-rule-mining/bigo_table.png')
