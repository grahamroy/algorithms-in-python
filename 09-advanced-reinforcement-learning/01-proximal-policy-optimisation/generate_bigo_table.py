"""Generate a Big-O / cost summary table image for the PPO article."""
...
ROWS = [
    ['Collect a rollout (N steps)',
     'O(N · W)',
     'O(N · d)',
     'one actor + critic pass per step'],
    ['Compute GAE advantages',
     'O(N)',
     'O(N)',
     'one backward pass over the rollout'],
    ['Probability ratio  r',
     'O(B · W)',
     'O(B)',
     'new vs old log-probs per minibatch'],
    ['Clipped update (per epoch)',
     'O(N · W)',
     'O(W)',
     'minibatch SGD on min(r·A, clip·A)'],
    ['Full iteration',
     'O(E · N · W)',
     'O(W + N·d)',
     'E epochs of reuse over the rollout'],
    ['Total memory',
     '—',
     'O(W + N·d)',
     'two nets + one rollout, no replay buffer'],
]

COL_WIDTHS = [5.8, 3.3, 3.3, 6.4]
...
fig, ax = plt.subplots(figsize=(1640/150, 850/150), dpi=150)
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Proximal Policy Optimisation Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'W = network weights, N = rollout steps, B = minibatch, E = epochs, '
        'd = state dim. More epochs E = more compute, fewer env steps.',
