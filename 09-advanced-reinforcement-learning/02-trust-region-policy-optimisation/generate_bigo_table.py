"""Generate a Big-O / cost summary table image for the TRPO article."""
...
ROWS = [
    ['Collect a rollout (N steps)',
     'O(N · W)',
     'O(N · d)',
     'actor + critic pass per step'],
    ['Surrogate gradient  g',
     'O(N · W)',
     'O(P)',
     'one backward pass; P = #params'],
    ['Fisher-vector product  F v',
     'O(N · W)',
     'O(P)',
     'finite-diff Jv + one backward'],
    ['Natural gradient (CG)',
     'O(k · N · W)',
     'O(P)',
     'k Fisher-vector products, no F'],
    ['Line search',
     'O(b · N · W)',
     'O(P)',
     'b backtracks, KL + surrogate check'],
    ['Total memory',
     '—',
     'O(W + N·d)',
     'never forms the P×P Fisher matrix'],
]

COL_WIDTHS = [5.9, 3.5, 3.0, 6.0]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Trust Region Policy Optimisation Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'W = network weights, N = rollout steps, P = #params, k = CG iters, '
        'b = backtracks, d = state dim. The CG solve is the cost of an exact step.',
