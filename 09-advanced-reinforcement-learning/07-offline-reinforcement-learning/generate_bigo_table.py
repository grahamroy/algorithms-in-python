"""Generate a Big-O / cost summary table image for the Offline RL article."""
...
ROWS = [
    ['Sample minibatch',
     'O(B)',
     'O(1)',
     'from the fixed, read-only dataset'],
    ['Twin critic update',
     'O(B · W)',
     'O(W)',
     'the same TD target as TD3'],
    ['Actor update + BC',
     'O(B · W)',
     'O(W)',
     '−λ·Q(s, π(s)) + (π(s) − a_data)²'],
    ['Full training',
     'O(G · B · W)',
     'O(W)',
     'G gradient steps; no rollouts at all'],
    ['Environment steps',
     '0',
     '—',
     'the defining feature of offline RL'],
    ['Total memory',
     '—',
     'O(W + N·d)',
     'the dataset IS the buffer, never grown'],
]

COL_WIDTHS = [5.8, 3.4, 3.0, 6.4]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'W = per-network weights, B = minibatch, G = gradient steps, '
        'N = dataset size, d = state dim. Training is pure computation — '
        'evaluation is what still needs the world.',
