"""Generate a Big-O / cost summary table image for the MCTS article."""
...
ROWS = [
    ['Selection (per node)',
     'O(b)',
     'O(1)',
     'UCT compare over b children'],
    ['Expansion',
     'O(b)',
     'O(1)',
     'add exactly one node per simulation'],
    ['Playout (simulation)',
     'O(L)',
     'O(1)',
    ...
    ['Backpropagation',
     'O(d)',
     'O(1)',
     'update N, W along the path'],
    ['One full search (a move)',
     'O(S·(d·b + L))',
     'O(S)',
     'S simulations; tree has <= S nodes'],
    ['Training required',
     'none',
     'none',
     'pure planning -- no parameters at all'],
]

COL_WIDTHS = [5.7, 3.9, 2.8, 6.2]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Monte Carlo Tree Search Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'S = simulations per move, b = branching factor, d = tree depth, '
        'L = playout length. Anytime: stop whenever and act on the best-visited move.',
