"""Generate a Big-O / cost summary table image for the SAC article."""
...
ROWS = [
    ['Sample  a ~ π(·|s)',
     'O(W)',
     'O(1)',
     'reparameterised: 2·tanh(μ + σε)'],
    ['Twin critic update',
     'O(B · W)',
     'O(W)',
     'soft target y = r + γ(minQ′ − α·logπ)'],
    ['Actor update',
     'O(B · W)',
     'O(W)',
     'dQ/da through the min critic into μ, σ'],
    ['Temperature update',
     'O(B)',
     'O(1)',
     'one scalar: entropy vs the target'],
    ['Store / sample transition',
     'O(B)',
     'O(N · d)',
    ...
    ['Total memory',
     '—',
     'O(W + N·d)',
     'five nets — no target actor needed'],
]

COL_WIDTHS = [5.4, 3.2, 3.0, 6.6]
...
fig, ax = plt.subplots(figsize=(1660/150, 850/150), dpi=150)
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'W = per-network weights, B = minibatch, N = replay capacity, d = state '
        "dim. TD3's costs with the target actor swapped for sampling + a "
        'one-parameter controller.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out = ('D:/Projects/Medium/algorithms-in-python/'
       '09-advanced-reinforcement-learning/05-soft-actor-critic/bigo_table.png')
