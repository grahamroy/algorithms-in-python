"""Generate a Big-O / cost summary table image for the DDPG article."""
...
ROWS = [
    ...
    ['Critic update (TD)',
     'O(B · W)',
     'O(W)',
     'target y = r + γ Q\'(s\', μ\'(s\'))'],
    ['Actor update (dQ/da)',
     'O(B · W)',
     'O(W)',
     'ascend Q through the critic'],
    ['Soft-update targets',
     'O(W)',
     'O(W)',
     "θ' ← τθ + (1−τ)θ', both nets"],
    ['Store / sample transition',
     'O(B)',
     'O(N · d)',
     'replay buffer of N transitions'],
    ['Total memory',
     '—',
     'O(W + N·d)',
     'four nets + a replay buffer'],
]

COL_WIDTHS = [5.6, 3.3, 3.0, 6.2]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Deep Deterministic Policy Gradient Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        ...
        'dim. Four nets (two trained, two slow targets); off-policy replay reuse.',
