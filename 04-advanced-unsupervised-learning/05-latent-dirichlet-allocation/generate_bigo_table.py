"""Generate a Big-O / cost summary table image for the LDA article."""
...
ROWS = [
    ['Variational EM, one iter',
     'O(D · N̄ · K)',
     'O(D · K + K · V)',
     'per-doc E-step + global M-step'],
    ['Full training (batch)',
     'O(I · D · N̄ · K)',
     'O(D · K + K · V)',
     'I = 50–200 iterations typically'],
    ['Online LDA (streaming)',
     'O(B · N̄ · K)',
     'O(B · K + K · V)',
     'B-doc mini-batches; scales to millions'],
    ['Collapsed Gibbs sampling',
     'O(S · total_tokens · K)',
     'O(total_tokens)',
     'S sweeps; slower but principled'],
    ['Infer new document',
     'O(N · K)',
     'O(K)',
     'one document, one variational pass'],
    ['Top-words extraction',
     'O(K · V · log V)',
     'O(K · top_n)',
     'sort each topic-word vector'],
]

COL_WIDTHS = [5.4, 4.0, 3.0, 5.6]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'D = documents, N̄ = avg words per doc, V = vocabulary, '
        'K = topics, I = iterations, B = mini-batch size.',
...
out = ('D:/Projects/Medium/algorithms-in-python/'
       ...
       '05-latent-dirichlet-allocation/bigo_table.png')
