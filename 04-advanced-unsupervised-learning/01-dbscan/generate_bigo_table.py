"""Generate a Big-O / cost summary table image for the DBSCAN article."""
...
ROWS = [
    ['Neighbour query (naive)',
     'O(n · d)',
     'O(n)',
     'scan over every other point'],
    ['Neighbour query (KD-tree)',
     'O(log n · d)',
     'O(n · d)',
     'works well for d ≲ 20'],
    ['Full fit (naive)',
     'O(n² · d)',
     'O(n²)',
     'one query per point + full distance matrix'],
    ['Full fit (KD-tree / ball tree)',
     'O(n · log n · d)',
     'O(n · k)',
     'sklearn default; scales to n ≈ 10⁶ in 2D'],
    ['Predict new point',
     'O(log n · d)',
     '—',
     'no native support; 1-NN against training'],
    ['Memory for clustering',
     '—',
     'O(n)',
     'one label per point; tiny model'],
]

COL_WIDTHS = [5.8, 3.6, 2.6, 6.0]
...
ax.text(TABLE_W / 2 + 1.0, caption_y,
        'n = data points, d = features, k = average neighbours per point.',
