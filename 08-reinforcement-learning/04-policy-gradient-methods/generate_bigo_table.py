"""Generate a Big-O / cost summary table image for the Policy Gradient article."""
...
ROWS = [
    ['Sample action  a ~ π(·|s)',
     'O(W)',
     'O(1)',
     'forward pass + sample from softmax'],
    ['Run one episode',
     'O(T · W)',
     'O(T · d)',
     'store the trajectory (s, a, r)'],
    ['Compute returns  G_t',
     'O(T)',
     'O(T)',
     'one backward pass over rewards'],
    ['Gradient update',
     'O(T · W)',
     'O(W)',
     'return-weighted log-π, one per episode'],
    ['Total memory',
     '—',
     'O(W + T·d)',
...
fig, ax = plt.subplots(figsize=(1600/150, 800/150), dpi=150)
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'REINFORCE (Policy Gradient) Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'W = network weights, T = steps per episode, d = state dim. On-policy: '
        'each trajectory is used once and discarded — small memory, low reuse.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out = ('D:/Projects/Medium/algorithms-in-python/'
       '08-reinforcement-learning/04-policy-gradient-methods/bigo_table.png')
