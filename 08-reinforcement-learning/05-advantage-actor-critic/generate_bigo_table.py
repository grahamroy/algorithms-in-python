"""Generate a Big-O / cost summary table image for the A2C article."""
...
ROWS = [
    ...
    ['Critic update',
     'O(T · W_c)',
     'O(W_c)',
     'regress V(s) toward the return'],
    ['Total memory',
     '—',
     'O(W + T·d)',
     'two nets, no replay buffer (on-policy)'],
]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Advantage Actor-Critic Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'W_a = actor weights, W_c = critic weights, W = W_a + W_c, '
        'T = steps/episode, d = state dim. Two networks, REINFORCE\'s order.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out = ('D:/Projects/Medium/algorithms-in-python/'
       '08-reinforcement-learning/05-advantage-actor-critic/bigo_table.png')
