"""Generate a Big-O / cost summary table image for the SARSA article."""
...
ROWS = [
    ['Choose action (ε-greedy)',
     'O(|A|)',
     'O(1)',
     'argmax over actions, or random'],
    ['TD update (one step)',
     'O(|A|)',
     'O(1)',
     'reads ONE Q(s′,a′) — no max'],
    ['One episode',
     'O(steps · |A|)',
     'O(1)',
     'steps until terminal or cap'],
    ['Full training',
     'O(E · steps · |A|)',
     'O(|S| · |A|)',
     'E episodes; Q-table dominates memory'],
    ['Q-table storage',
     '—',
     'O(|S| · |A|)',
     'one value per state-action pair'],
    ['Act greedily (serving)',
     'O(|A|)',
     'O(|S| · |A|)',
     'argmax over the learned table'],
]

COL_WIDTHS = [5.4, 3.6, 3.0, 5.6]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'Identical to Q-Learning — same tabular TD method. The only per-step '
        'difference: SARSA reads one Q(s′,a′) instead of a max.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out = ('D:/Projects/Medium/algorithms-in-python/'
       '08-reinforcement-learning/02-sarsa/bigo_table.png')
