"""Generate a Big-O / cost summary table image for the Deep Q-Networks article."""
...
ROWS = [
    ['Forward pass  Q(s, ·; θ)',
     'O(W)',
     'O(W)',
     'W = number of network weights'],
    ['Choose action (ε-greedy)',
     'O(W)',
     'O(1)',
     'one forward pass + argmax over |A|'],
    ['Gradient step (minibatch)',
     'O(B · W)',
     'O(B · W)',
     'B = batch; forward + backprop'],
    ['Store / sample transition',
     'O(B)',
     'O(N · d)',
     'replay ring of N transitions, dim d'],
    ['Target sync (every C steps)',
     'O(W)',
     'O(W)',
     'copy online weights into target net'],
    ['Total memory',
     '—',
     'O(W + N·d)',
     'independent of |S| — the table wall is gone'],
]

COL_WIDTHS = [5.6, 3.3, 3.3, 6.2]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'W = network weights, B = batch size, N = replay capacity, '
        'd = state dim, |A| = actions. Cost scales with the network, not |S|.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out = ('D:/Projects/Medium/algorithms-in-python/'
       '08-reinforcement-learning/03-deep-q-networks/bigo_table.png')
