"""Generate a Big-O / cost summary table image for the SVM article."""
...
ROWS = [
    ['Training, linear SVM',
     'O(n · d)',
     'O(n · d)',
     'LIBLINEAR / hinge SGD; scales to n=10⁶'],
    ['Training, kernel SVM',
     'O(n² · d) to O(n³ · d)',
     'O(n²)',
     'Gram matrix n×n; impractical past n≈10⁵'],
    ['Predict, linear SVM',
     'O(d)',
     'O(d)',
     'single dot product; very fast'],
    ['Predict, kernel SVM',
     'O(n_SV · d)',
     'O(n_SV · d)',
     'sum over support vectors; sparse model'],
    ['Probability calibration',
     'O(n · CV)',
     'O(n)',
     'Platt scaling: extra training pass'],
]

COL_WIDTHS = [4.6, 4.4, 3.2, 5.6]
...
ax.text(TABLE_W / 2 + 1.0, caption_y,
        'n = training examples, d = features, n_SV = number of support vectors, '
...
out_path = ('D:/Projects/Medium/algorithms-in-python/'
            ...
            '03-support-vector-machines/bigo_table.png')
