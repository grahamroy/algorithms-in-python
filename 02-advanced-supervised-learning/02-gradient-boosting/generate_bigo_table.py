"""Generate a Big-O / cost summary table image for the Gradient Boosting article."""
...
ROWS = [
    ['Training (exact splits)',
     'O(M · n · d · depth)',
     'O(M · nodes)',
     'sequential across M; per-tree like CART'],
    ['Training (histogram mode)',
     'O(M · n · d)',
     'O(M · nodes)',
     'LightGBM / XGBoost hist; quantile bins'],
    ['Predict one example',
     'O(M · depth)',
     'O(M · nodes)',
     'walk every tree; same cost shape as RF'],
    ['Model size',
     '—',
     'O(M · nodes)',
     'megabytes; truncate later trees if needed'],
    ['Parallelism across trees',
     '—',
     '—',
     'cannot — boosting is sequential by design'],
]

COL_WIDTHS = [5.6, 4.0, 3.0, 5.4]
...
ax.text(TABLE_W / 2 + 1.0, caption_y,
        'M = boosting rounds, n = training examples, d = features, '
...
out_path = ('D:/Projects/Medium/algorithms-in-python/'
            ...
            '02-gradient-boosting/bigo_table.png')
