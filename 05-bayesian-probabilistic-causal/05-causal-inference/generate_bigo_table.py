"""Generate a Big-O / cost summary table image for the Causal Inference article."""
...
ROWS = [
    ['Naive diff in means',
     'O(n)',
     'O(1)',
     'fast but biased under confounding'],
    ['Regression adjustment',
     'O(model fit)',
     'O(model size)',
     'cost depends on chosen regressor'],
    ['Inverse propensity weighting',
     'O(model fit)',
     'O(n)',
     'fit propensity classifier + weighted avg'],
    ['Doubly-robust estimator',
     'O(2 × model fit)',
     'O(2 × model size)',
     'unbiased if either model is correct'],
    ['Causal forest (Athey-Wager)',
     'O(N · n · log n)',
     'O(N · nodes)',
     'N trees; targets heterogeneous effects'],
    ['Synthetic controls',
     'O(n²)',
     'O(n²)',
     'one-time interventions; small n'],
]

COL_WIDTHS = [5.4, 3.8, 3.0, 5.6]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'n = units, N = trees, "model fit" = whatever ML method is plugged in '
        '(linear regression, GBM, neural net).',
...
out = ('D:/Projects/Medium/algorithms-in-python/'
       ...
       '05-causal-inference/bigo_table.png')
