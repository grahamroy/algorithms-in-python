"""Generate a Big-O / cost summary table image for the MCMC article."""
...
ROWS = [
    ['One log-posterior eval',
     'O(n · d)',
     'O(d)',
     'evaluates likelihood over all n data'],
    ['One MH iteration',
     'O(n · d)',
     'O(d)',
     'one log-posterior + acceptance check'],
    ['Full chain (N samples)',
     'O(N · n · d)',
     'O(N · d)',
     'N = 10⁴–10⁶ typical'],
    ['Multi-chain (M chains)',
     'O(M · N · n · d)',
     'O(M · N · d)',
     'M = 4 standard; parallelisable'],
    ['HMC iteration',
     'O(L · n · d)',
     'O(d)',
     'L leapfrog steps, gradient-based; far higher ESS'],
    ['NUTS iteration',
     'O(2^j · n · d)',
     'O(d)',
     'j = adaptive depth, typically 8–10'],
]

COL_WIDTHS = [5.0, 3.4, 2.4, 6.4]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'n = data points, d = parameter dimension, N = samples per chain, '
        'M = chains, L = HMC leapfrog steps, j = NUTS tree depth.',
...
out = ('D:/Projects/Medium/algorithms-in-python/'
       ...
       '02-markov-chain-monte-carlo/bigo_table.png')
