"""Generate a Big-O / cost summary table image for the PPL article."""
...
HEADERS = ['Stage', 'Cost', 'Memory', 'Notes']
ROWS = [
    ['Model compilation',
     'O(model size)',
     'O(model size)',
     '1-time; seconds for typical models'],
    ['Auto-differentiation',
     'O(graph nodes)',
     'O(graph nodes)',
     'gradient computed for free'],
    ['NUTS per iteration',
     'O(2^j · n · d)',
     'O(d)',
     'j = adaptive tree depth (~8 typical)'],
    ['Full inference (NUTS)',
     'O(N · 2^j · n · d / M)',
     'O(N · d)',
     'M parallel chains, N draws each'],
    ['Variational alternative',
     'O(I · n · d)',
     'O(d)',
     'pm.fit() option; much faster, approximate'],
    ['Posterior predictive',
     'O(N_pp · n · d)',
     'O(N_pp · n)',
     'simulate datasets from fitted posterior'],
]

COL_WIDTHS = [5.0, 4.0, 3.0, 5.6]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'n = data points, d = parameters, j = NUTS tree depth, '
        'N = draws per chain, M = chains, I = VI iterations.',
...
out = ('D:/Projects/Medium/algorithms-in-python/'
       ...
       '04-probabilistic-programming/bigo_table.png')
