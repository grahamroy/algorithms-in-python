"""Generate a Big-O / cost summary table image for the VI article."""
...
HEADERS = ['Method', 'Cost', 'Memory', 'Notes']
ROWS = [
    ['Coordinate-ascent VI',
     'O(I · n · d)',
     'O(d)',
     'mean-field; closed-form per-coord updates'],
    ['Stochastic VI (SVI)',
     'O(I · B · d)',
     'O(d)',
     'mini-batch B; scales to millions of points'],
    ['Black-box VI',
     'O(I · S · d)',
     'O(S · d)',
     'S Monte Carlo samples per gradient step'],
    ['Reparameterised VI (VAE)',
     'O(E · n/B · L · d²)',
     'O(L · d²)',
     'amortised; encoder learns q(z | x)'],
    ['MCMC (for comparison)',
     'O(N · n · d)',
     'O(N · d)',
     'N ≫ I; far slower per equivalent accuracy'],
    ['Predict / sample from q',
     'O(d)',
     'O(d)',
     'sub-millisecond after training'],
]

COL_WIDTHS = [5.4, 3.8, 2.8, 5.6]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'n = data points, d = parameters, I = iterations, '
        'B = batch size, S = MC samples, E = epochs, L = layers.',
...
out = ('D:/Projects/Medium/algorithms-in-python/'
       ...
       '03-variational-inference/bigo_table.png')
