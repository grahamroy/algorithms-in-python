"""Generate a results comparison table image for the VI article."""
...
# Table data
HEADERS = ['Method', 'intercept (mean ± std)', 'slope (mean ± std)', 'time (s)']
ROWS = [
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Bayesian linear regression: three methods agree',
...
for r, row in enumerate(ROWS):
    ...
    ax.text(col_x(2) + COL_WIDTHS[2] / 2, row_y + ROW_H / 2, row[2],
            fontsize=11, ha='center', va='center',
            color=TEXT_COLOR, fontfamily='DejaVu Sans Mono', zorder=2)
    # Time (mono, highlighted-ish)
    ax.text(col_x(3) + COL_WIDTHS[3] / 2, row_y + ROW_H / 2, row[3],
            fontsize=11, ha='center', va='center',
            color='#059669', fontfamily='DejaVu Sans Mono',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        ...
        'Training set: 200 noisy points, σ = 0.5.',
...
out = ('D:/Projects/Medium/algorithms-in-python/'
       ...
       '03-variational-inference/results_table.png')
