"""Generate a Big-O / cost summary table image for the Transformers
article."""
...
ROWS = [
    ['Attention, per layer',
     'O(T² · d)',
     'O(h · T²)',
     "Part 4's bill, paid in full"],
    ['MLP block, per layer',
     'O(T · d · d_ff)',
     'O(d · d_ff)',
     'most of the parameters live here'],
    ['LayerNorm + residual',
     'O(T · d)',
     '—',
     'cheap bookkeeping, priceless function'],
    ['Full forward',
     'O(L·(T²·d + T·d²))',
     'O(L · h · T²)',
     'which term wins depends on T vs d'],
    ['Training step',
     '~3x forward',
     'O(L · T · d)',
     'parallel over ALL positions at once'],
    ['Generate one token',
     'O(T · d · L)',
     'O(L · T · d)',
     'with a KV-cache; still sequential'],
]

COL_WIDTHS = [5.0, 4.6, 3.2, 7.2]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'T = context length, d = width, d_ff = 4d, h = heads, L = layers. '
        'Parameters O(L · d²), independent of T. Training is parallel; '
        'generation is not.',
...
out = ('D:/Projects/Medium/algorithms-in-python/'
       '11-deep-learning-architectures/05-transformers/bigo_table.png')
