"""Generate a Big-O / cost summary table image for the GAN article."""
...
ROWS = [
    ...
    ['Detective step',
     'O(b · L · h²)',
     'O(b · h)',
     'binary cross-entropy (Part 1)'],
    ['Forger step',
     'O(b · L · h²)',
     'O(b · h)',
     'backprop THROUGH the detective'],
    ['Full training step',
     'O(b · L · h²)',
     'O(L · h²)',
     'three passes: real, fake, forger'],
    ['Evaluation (FID-class)',
     'O(N · d² + d³)',
     'O(d²)',
     'no loss to trust; score samples'],
    ['vs. diffusion sample',
     'O(T · L · h²)',
     'O(L · h²)',
     'T sequential passes (Part 9)'],
]

COL_WIDTHS = [5.2, 4.0, 3.0, 7.4]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Generative Adversarial Network Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'h = layer width, L = layers, b = batch, N = samples scored, '
        'd = score-net feature dim, T = diffusion steps. One-pass '
        "sampling is the GAN's lasting price advantage.",
