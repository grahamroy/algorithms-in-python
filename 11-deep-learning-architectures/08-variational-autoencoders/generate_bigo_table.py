"""Generate a Big-O / cost summary table image for the VAE article."""
...
ROWS = [
    ['Sampling',
     'O(L · h²)',
     'O(L · h²)',
     'one decoder pass, no detective'],
    ['Enc / dec step',
     'O(b · L · h²)',
     'O(b · h)',
     'one pass each, like Part 1'],
    ['KL term (Gaussian)',
     'O(b · d_z)',
     'O(b · d_z)',
     'closed form -- the diagonal payoff'],
    ['Reparam gradient',
     'O(b · d_z)',
     'O(b · d_z)',
     'exact; REINFORCE = 6x the noise'],
    ['Evaluation (ELBO)',
     'O(N · L · h²)',
     'O(b · h)',
     'the model grades itself'],
    ['vs. GAN training step',
     'O(b · L · h²)',
     'O(L · h²)',
     'three passes and two networks'],
]

COL_WIDTHS = [5.6, 4.0, 3.0, 7.4]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Variational Autoencoder Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'h = layer width, L = layers, b = batch, d_z = latent dims, '
        'N = eval points. The ELBO is a comparable number the model '
        'produces itself -- the GAN had no such line.',
