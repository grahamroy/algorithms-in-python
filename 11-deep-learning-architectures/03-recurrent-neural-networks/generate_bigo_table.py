"""Generate a Big-O / cost summary table image for the RNN/LSTM article."""
...
ROWS = [
    ...
    ['Forward, full sequence',
     'O(T · H²)',
     'O(T · H)',
     'activations kept -- BPTT eats them'],
    ['BPTT backward',
     'O(T · H²)',
     'O(T · H)',
     'the forward, reversed through time'],
    ['LSTM cell step',
     'O(4 · H²)',
     'O(4 · H²)',
     'four gate blocks, one fused matmul'],
    ['Full training',
     'O(E · N · T · H²)',
     'O(N · T · H)',
     'linear in T everywhere'],
    ['Inference',
     'O(T · H²)',
     'O(H)',
     'constant state -- but SEQUENTIAL in T'],
]

COL_WIDTHS = [4.9, 4.2, 3.2, 7.0]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'T = sequence length, H = hidden units, D = input dim, N = batch, '
        'E = epochs. Step t cannot start before t-1 ends -- the line that '
        'ended an era.',
