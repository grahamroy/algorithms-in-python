"""Generate a Big-O / cost summary table image for the MLP article."""
...
ROWS = [
    ['Forward pass',
     'O(B · W)',
     'O(B · H)',
     'keep activations for backprop'],
    ['Backward pass',
     'O(B · W)',
     'O(W)',
     'the forward pass, run in reverse'],
    ['Adam step',
     'O(W)',
     'O(2 · W)',
    ...
    ['Full training',
     'O(E · B · W)',
     'O(B · H + W)',
     'E epochs; backward ~ forward cost'],
    ['Gradient check',
     'O(P · B · W)',
     'O(1)',
     'two forwards per parameter; a test'],
    ['Predict',
     'O(W)',
     '—',
     'per point; one forward, nothing kept'],
]

COL_WIDTHS = [4.4, 4.0, 3.4, 6.9]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Multi-Layer Perceptron Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'B = batch size, W = weights, H = hidden units, E = epochs, '
        'P = parameters checked. The backward pass costing no more than '
        'the forward is why backprop won.',
