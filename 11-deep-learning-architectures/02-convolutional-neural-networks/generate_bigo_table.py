"""Generate a Big-O / cost summary table image for the CNN article."""
...
ROWS = [
    ['im2col',
     'O(N · P · k²)',
     'O(N · P · k²)',
     'copy pixels k² times; buy one matmul'],
    ['Conv forward',
     'O(N · P · k² · F)',
     'O(k² · F)',
     'params independent of image size'],
    ['Max-pool 2x2',
     'O(N · P · F)',
     '—',
     'no parameters; backward routes argmax'],
    ['Conv backward',
     'O(N · P · k² · F)',
     'O(k² · F)',
     "the forward, reversed (Part 1's law)"],
    ['Full training',
     'O(E · N · P · k² · F)',
     'O(N · P · k²)',
     'im2col buffer dominates memory'],
    ['Predict',
     'O(P · k² · F)',
     '—',
     'per image; one forward'],
]

COL_WIDTHS = [4.2, 4.9, 3.3, 7.0]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Convolutional Neural Network Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'N = batch, P = positions (12x12 here), k = kernel size, '
        'F = filters, E = epochs. Compute scales with the image; '
        'the parameters never do.',
