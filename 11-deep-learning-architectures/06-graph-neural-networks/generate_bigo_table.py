"""Generate a Big-O / cost summary table image for the GNN article."""
...
ROWS = [
    ...
    ['GAT layer',
     'O(E·d + E)',
     'O(E)',
     'score + softmax over neighbours'],
    ['L layers',
     'O(L · E · d)',
     'O(L · d²)',
     'L hops of receptive field'],
    ['GraphSAGE step',
     'O(b · k^L · d)',
     'O(b · k^L)',
     'sample k neighbours; web-scale'],
    ['vs. attention on n',
     'O(n² · d)',
     'O(n²)',
     'the complete graph, for comparison'],
]

COL_WIDTHS = [5.0, 4.2, 3.0, 7.4]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'n = nodes, E = edges, d = feature width, h = hidden, L = layers, '
        'b = batch, k = sampled neighbours. Sparse graphs make E << n² -- '
        "the GNN's price advantage.",
