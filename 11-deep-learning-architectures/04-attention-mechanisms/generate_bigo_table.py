"""Generate a Big-O / cost summary table image for the Attention article."""
...
ROWS = [
    ['Embed + positions',
     'O(T · V · d)',
     'O(T · d)',
     'sinusoids: order without parameters'],
    ['One query lookup',
     'O(T · d_k)',
     'O(T)',
     "Bahdanau's setting -- distance-free"],
    ['Self-attention layer',
     'O(T² · d_k)',
     'O(T²)',
     'every position a query -- the bill'],
    ['Backward pass',
     'same as forward',
     'O(T · d)',
     "Part 1's law, unbroken"],
    ['Full training',
     'O(E · N · T · d · d_k)',
     'O(N · T · d)',
     'single-query model; fully parallel in t'],
    ['Parameters',
     'O(V·d + d·d_k)',
     '—',
     'independent of T, as every part so far'],
]

COL_WIDTHS = [4.9, 4.9, 3.0, 7.2]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'T = sequence length, d = model width, d_k = head width, '
        'V = vocab, N = batch, E = epochs. Distance is free; PAIRS are not.',
