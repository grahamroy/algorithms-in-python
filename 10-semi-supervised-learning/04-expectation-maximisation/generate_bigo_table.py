"""Generate a Big-O / cost summary table image for the EM article."""
...
ROWS = [
    ['E-step (responsibilities)',
     'O(n · k · d²)',
     'O(n · k)',
     'a Gaussian density per point per class'],
    ['M-step (weighted refit)',
     'O(n · k · d²)',
     'O(k · d²)',
     'weighted means + covariances, one pass'],
    ['One iteration',
     'O(n · k · d²)',
     'O(n · k)',
     'closed form -- no learning rate'],
    ['Full run',
     'O(I · n · k · d²)',
     'O(n · d)',
     'I iterations to the plateau (~30 here)'],
    ['The guarantee',
     '—',
     '—',
     'observed log-likelihood never decreases'],
    ['Labels used',
     'soft',
     '—',
     'unlabelled points count fractionally'],
]

COL_WIDTHS = [5.6, 4.2, 3.0, 6.4]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Expectation-Maximisation Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'n = points, k = classes, d = dimensions, I = iterations. '
        'The guarantee is about likelihood under YOUR model -- '
        'not about the truth.',
...
out = ('D:/Projects/Medium/algorithms-in-python/'
       '10-semi-supervised-learning/04-expectation-maximisation/bigo_table.png')
