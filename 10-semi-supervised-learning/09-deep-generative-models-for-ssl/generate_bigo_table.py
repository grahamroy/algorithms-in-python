"""Generate a Big-O / cost summary table image for the Deep Generative
Models for SSL article."""
...
ROWS = [
    ['Gaussian mixture EM',
     'O(I · N · d²)',
     'O(d²)',
     'closed-form M-step, dozens of iters'],
    ['Warm start',
     'O(E_w · l · W)',
     'O(W)',
     'own labels only; fixes identifiability'],
    ['E-step (ELBOs)',
     'O(C · K · N · W)',
     'O(N)',
     'K Monte-Carlo samples per component'],
    ['M-step (VAE steps)',
     'O(C · N · W)',
     'O(W)',
     'one weighted forward/backward each'],
    ['Full VAE mixture',
     'O(E · C · N · W)',
     'O(C · W)',
     'thousands of steps; the story can bend'],
    ['Predict / generate',
     'O(C · K · W)',
     '—',
     'argmax ELBO / one decoder pass'],
]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Deep Generative SSL Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'N = all points, l = labelled, C = classes, W = network weights, '
        'K = MC samples, E / E_w = training / warm-start epochs, '
        'I = EM iterations, d = input dim.',
