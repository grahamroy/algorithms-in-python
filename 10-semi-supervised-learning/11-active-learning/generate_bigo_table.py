"""Generate a Big-O / cost summary table image for the Active Learning
article."""
...
ROWS = [
    ['Retrain (this learner)',
     'O(n³)',
     'O(n²)',
     "Part 5's harmonic solve, per query"],
    ['Uncertainty scores',
     'O(U)',
     'O(U)',
     'margin over the pool, from F'],
    ['Information density',
     'O(U)',
     'O(U)',
     'margin x precomputed density'],
    ['Farthest-first',
     'O(U · B)',
     'O(U)',
     'distance to the labelled set'],
    ['Full loop, budget B',
     'O(B · n³)',
     'O(n²)',
     'retrain dominates; batch-mode amortises'],
    ['The oracle',
     'B queries',
     '—',
     'the REAL cost -- the loop exists for this'],
]

COL_WIDTHS = [5.2, 3.4, 2.9, 7.2]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'n = all points (graph), U = unlabelled pool, B = label budget. '
        'Compute is priced in seconds; the oracle is priced in humans.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out = ('D:/Projects/Medium/algorithms-in-python/'
       '10-semi-supervised-learning/11-active-learning/bigo_table.png')
