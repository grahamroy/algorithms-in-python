"""Generate a Big-O / cost summary table image for the Graph-Based Methods article."""
...
ROWS = [
    ['The meter  fᵀLf',
     'O(E)',
     'O(E)',
     'one pass over the E edges'],
    ['Fiedler vector',
     'O(E · I)',
     'O(n)',
     'sparse eigensolver; I iterations'],
    ['Spreading solve',
     'O(n_u³) / ~O(E)',
     'O(E)',
     'dense solve, or CG on the sparse graph'],
    ['Leave-one-out audit',
     'O(1) each',
     'O(1)',
     'rank-one update once the solve is done'],
    ['Graph build (shared)',
     'O(n² · d)',
     'O(E)',
     'as in Part 5; ANN search cuts it at scale'],
    ['Training / parameters',
     'none',
     'none',
     'linear algebra on one sparse matrix'],
]

COL_WIDTHS = [5.2, 4.2, 2.9, 6.4]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Graph-Based Methods Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'n = nodes, n_u = unlabelled, E = edges (n·k for a k-NN graph), '
        'I = eigensolver iterations, d = dims. One matrix, many questions.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out = ('D:/Projects/Medium/algorithms-in-python/'
       '10-semi-supervised-learning/06-graph-based-methods/bigo_table.png')
