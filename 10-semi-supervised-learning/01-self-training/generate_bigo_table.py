"""Generate a Big-O / cost summary table image for the Self-Training article."""
...
ROWS = [
    ['Fit the base model',
     'O(F)',
     'O(M)',
     'whatever your classifier costs'],
    ['Score the pool',
     'O(U · P)',
     'O(U)',
     'U pool points, P = predict cost'],
    ['Promote confident points',
     'O(U)',
     'O(1)',
     'threshold test + move to train set'],
    ['One round',
     'O(F + U · P)',
     'O(M + U)',
     'fit, score, promote'],
    ['Full self-training',
     'O(R · (F + U·P))',
     'O(N · d)',
     'R rounds until nothing clears τ'],
    ['Labels gained',
     '—',
     'up to U',
     'here: 8 real labels -> 489 pseudo'],
]

COL_WIDTHS = [5.6, 3.9, 3.0, 6.0]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        ...
        'U = unlabelled pool, R = rounds, N = all points. A wrapper: it '
        'multiplies your model\'s cost by R.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out = ('D:/Projects/Medium/algorithms-in-python/'
       '10-semi-supervised-learning/01-self-training/bigo_table.png')
