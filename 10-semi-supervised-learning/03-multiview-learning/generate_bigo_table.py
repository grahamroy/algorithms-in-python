"""Generate a Big-O / cost summary table image for the Multiview article."""
...
ROWS = [
    ['Covariance matrices',
     'O(n · d²)',
     'O(d²)',
     'one pass over the n unlabelled pairs'],
    ['Whitening (eigh ×2)',
     'O(d³)',
     'O(d²)',
     'on the feature dim, not the sample count'],
    ['SVD of the cross-term',
     'O(d³)',
     'O(d²)',
     'directions + canonical correlations'],
    ['Project a point',
     'O(d · k)',
     'O(d · k)',
     'a matrix multiply; k components kept'],
    ['Downstream classifier',
     'tiny',
     'tiny',
     'few labels, k dims instead of d'],
    ['Labels consumed',
     '0',
     '—',
     'the representation is label-free'],
]

COL_WIDTHS = [5.3, 3.4, 3.0, 6.7]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Canonical Correlation Analysis Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'n = unlabelled pairs, d = view dimension, k = components kept. '
        'Closed-form linear algebra: no iterations, no learning rates — '
        'and no labels.',
...
out = ('D:/Projects/Medium/algorithms-in-python/'
       '10-semi-supervised-learning/03-multiview-learning/bigo_table.png')
