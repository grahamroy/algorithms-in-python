"""Generate a Big-O / cost summary table image for the TSVM article."""
...
ROWS = [
    ['Random feature map',
     'O(N · D · d)',
     'O(N · D)',
     'one-time; approximates the RBF kernel'],
    ['(T)SVM epoch',
     'O(N · D)',
     'O(D)',
     'hinge + hat subgradients, full batch'],
    ['Balance pin',
     'O(N)',
     'O(1)',
     'bias set analytically -- no free b'],
    ['One restart',
     'O(E · N · D)',
     'O(D)',
     'annealed C_u over E epochs'],
    ['Full TSVM',
     'O(R · E · N · D)',
     'O(N · D)',
     'R restarts, keep the lowest objective'],
    ['Predict',
     'O(D)',
     '—',
     'per point; induction survives'],
]

COL_WIDTHS = [4.6, 4.1, 3.0, 7.0]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'N = pool size, D = random features, d = input dim, E = epochs, '
        'R = restarts. R exists purely because the hat is not a bowl.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out = ('D:/Projects/Medium/algorithms-in-python/'
       '10-semi-supervised-learning/10-transductive-svm/bigo_table.png')
