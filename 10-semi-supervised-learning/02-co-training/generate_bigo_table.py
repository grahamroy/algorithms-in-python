"""Generate a Big-O / cost summary table image for the Co-Training article."""
...
ROWS = [
    ['Fit both base models',
     'O(2F)',
     'O(2M)',
     'one classifier per view'],
    ['Nominate, per view',
     'O(2 · U · P)',
     'O(U)',
     'each judge nominates independently'],
    ['Veto check',
     'O(U)',
     'O(1)',
     'compare nominations; drop conflicts'],
    ['One round',
     'O(2(F + U·P))',
     'O(2M + U)',
     'twice self-training, plus the check'],
    ['Full co-training',
     'O(R · 2(F + U·P))',
     'O(N·(dA+dB))',
     'R rounds until nominations stop'],
    ['The veto dividend',
     '—',
     '—',
     'here: 51 wrong nominations caught'],
]

COL_WIDTHS = [5.0, 4.7, 3.7, 5.6]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'F / P / M = the base classifier\'s fit / predict / memory costs, '
        'U = unlabelled pool, R = rounds, dA/dB = view dims. '
        'Twice the model cost, for an error check one model cannot have.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out = ('D:/Projects/Medium/algorithms-in-python/'
       '10-semi-supervised-learning/02-co-training/bigo_table.png')
