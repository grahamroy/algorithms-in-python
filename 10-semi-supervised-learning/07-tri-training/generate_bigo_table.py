"""Generate a Big-O / cost summary table image for the Tri-Training article."""
...
ROWS = [
    ['Bootstrap ensemble',
     'O(3 · n_l)',
     'O(3 · n_l)',
     'three resamples; unstable learner'],
    ['Teachers label pool',
     'O(3 · U · P)',
     'O(U)',
     'each pair scores every point'],
    ['Measure teacher error',
     'O(3 · n_l · P)',
     'O(1)',
     'on the labelled set, where they agree'],
    ['Gate + subsample',
     'O(U)',
     'O(1)',
     'admit only if e · |batch| shrinks'],
    ['Full tri-training',
     'O(R · 3(F + U·P))',
     'O(3 · N · d)',
     'gate closing = natural stopping rule'],
    ['Predict',
     'O(3 · P)',
     '—',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'n_l = labelled points, U = unlabelled pool, F / P = base fit / '
        'predict cost, R = rounds, N = all points. Three self-trainings, '
        'plus the admission policy that makes them safe.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out = ('D:/Projects/Medium/algorithms-in-python/'
       '10-semi-supervised-learning/07-tri-training/bigo_table.png')
