"""Generate a Big-O / cost summary table image for the VAT article."""
...
ROWS = [
    ['Supervised step',
     'O(B_l · W)',
     'O(W)',
     'plain forward / backward on the labels'],
    ['Random probe pass',
     'O(B · W)',
     'O(B · d)',
     'nudge by tiny xi·d, one forward pass'],
    ['KL backprop to input',
     'O(B · W)',
     'O(B · d)',
     'output error is just q - p; carried to x'],
    ['Attacked penalty pass',
     'O(B · W)',
     'O(W)',
     'forward / backward at x + r_adv'],
    ['Full VAT epoch',
     'O(3 · B · W)',
     'O(W)',
     'about 3x supervised; no extra params'],
    ['Predict',
     'O(W)',
     '—',
     'the regulariser vanishes at test time'],
]

COL_WIDTHS = [4.6, 3.6, 3.0, 7.4]
...
fig, ax = plt.subplots(figsize=(1700/150, 850/150), dpi=150)
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Virtual Adversarial Training Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'B_l = labelled batch, B = full batch (labelled + unlabelled), '
        'W = network weights, d = input dimension. VAT is a constant '
        'factor on training and free at inference.',
