"""Generate a Big-O summary table image for the trees article."""
...
ROW_ODD = '#FFFFFF'
BORDER = '#E2E8F0'
SEARCH_COLOR = '#059669'   # green
INSERT_COLOR = '#3b82f6'   # blue
NOTE_COLOR = '#6B7280'

# ── Table data ──
# Five tree variants compared on the operations that actually distinguish them.
HEADERS = ['Tree variant', 'Search', 'Insert', 'Typical use']
ROWS = [
    ['Balanced BST',        'O(log n)',      'O(log n)',     'in-memory ordered keys (red-black, AVL)'],
    ['Binary heap',         'O(1) peek min', 'O(log n)',     'priority queues, top-k selection'],
    ['KD-tree (k <= 20)',   'O(log n) avg',  'O(log n)',     'low-D nearest-neighbour search'],
    ['B-tree (fan-out f)',  'O(log_f n)',    'O(log_f n)',   'database and filesystem indexes'],
    ['Trie',                'O(L)',          'O(L)',         'prefix lookup, autocomplete (L = key length)'],
]

# ── Layout parameters ──
COL_WIDTHS = [4.0, 3.2, 3.0, 7.5]  # relative widths
...
title_y = TABLE_H + 1.3
ax.text(TABLE_W / 2 + 1.0, title_y, 'Tree Complexity --- Five Common Variants',
...
for r, row in enumerate(ROWS):
    ...
    # Tree variant (left-aligned, bold)
    ...
    # Search (green, monospace)
    ax.text(col_x(1) + COL_WIDTHS[1] / 2, row_y + ROW_H / 2, row[1],
            fontsize=11, ha='center', va='center',
            color=SEARCH_COLOR, fontfamily='DejaVu Sans Mono', zorder=2)
    # Insert (blue, monospace)
    ax.text(col_x(2) + COL_WIDTHS[2] / 2, row_y + ROW_H / 2, row[2],
            fontsize=11, ha='center', va='center',
            color=INSERT_COLOR, fontfamily='DejaVu Sans Mono', zorder=2)
    # Notes (left-aligned, italic, subtle)
...
ax.text(TABLE_W / 2 + 1.0, caption_y,
        'n = items in the tree.  Balanced operations assume a self-balancing implementation.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out_path = 'D:/Projects/Medium/algorithms-in-python/00-foundations/08-trees/bigo_table.png'
