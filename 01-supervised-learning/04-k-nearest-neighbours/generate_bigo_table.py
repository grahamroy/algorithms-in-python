"""Generate a Big-O / cost summary table image for the KNN article."""
...
ROWS = [
    ['Training (fit)',
     'O(n · d)',
     'O(n · d)',
     'just stores the training set; no parameters fit'],
    ['Predict, brute force',
     'O(n · d)',
     'O(n)',
     'distance to every training point'],
    ['Predict, KD-tree (low d)',
     'O(d · log n)',
     'O(n · d)',
     'good for d ≲ 20; degrades above that'],
    ['Predict, HNSW (any d)',
     'O(log n) approx.',
     'O(n · d)',
     'see Foundations Part 12; trades exactness for speed'],
    ['Model size',
     '—',
     'O(n · d)',
...
ax.text(TABLE_W / 2 + 1.0, title_y,
        'K-Nearest Neighbours Complexity',
...
ax.text(TABLE_W / 2 + 1.0, caption_y,
        ...
        'Predict cost is per query example.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out_path = 'D:/Projects/Medium/algorithms-in-python/01-supervised-learning/04-k-nearest-neighbours/bigo_table.png'
