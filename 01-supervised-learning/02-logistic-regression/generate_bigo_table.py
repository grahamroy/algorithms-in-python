"""Generate a Big-O / cost summary table image for the logistic regression article."""
...
ROWS = [
    ['Forward pass per epoch',
     'O(n \u00b7 d)',
     'O(n \u00b7 d)',
     'one matrix-vector product + sigmoid'],
    ['Gradient per epoch',
     'O(n \u00b7 d)',
     'O(d)',
     '(1/n) X\u1d40 (p\u0302 - y)'],
    ['Full training (E epochs)',
     'O(E \u00b7 n \u00b7 d)',
     'O(n \u00b7 d)',
     'E typically 10\u00b2 to 10\u2074'],
    ['Prediction (per example)',
     'O(d)',
     'O(d)',
     'one dot product + sigmoid'],
    ['Model parameters',
     '\u2014',
     'O(d)',
     'd + 1 floats; tiny vs the data matrix'],
    ['Sparse-feature path',
     'O(E \u00b7 nnz)',
     'O(nnz + d)',
     'CSR design matrix; see Foundations Part 11'],
]

# ── Layout parameters ──
COL_WIDTHS = [4.6, 3.0, 2.6, 6.4]
...
ax.text(TABLE_W / 2 + 1.0, caption_y,
        'n = examples, d = features, E = training epochs, nnz = non-zero entries in a sparse design matrix.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out_path = 'D:/Projects/Medium/algorithms-in-python/01-supervised-learning/02-logistic-regression/bigo_table.png'
