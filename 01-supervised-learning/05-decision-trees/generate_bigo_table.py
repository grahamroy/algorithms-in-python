"""Generate a Big-O / cost summary table image for the Decision Trees article."""
...
ROWS = [
    ['Training (typical)',
     'O(n · d · depth)',
     'O(nodes)',
     'every depth level scans every feature'],
    ['Training (worst case)',
     'O(n² · d)',
     'O(nodes)',
     'unbalanced tree on adversarial data'],
    ['Predict one example',
     'O(depth)',
     'O(nodes)',
     'walk root → leaf; tiny constant'],
    ['Model size',
     '—',
     'O(nodes)',
     'a compressed flowchart, KB-sized'],
    ['Feature importance',
     'O(nodes)',
     'O(d)',
     'sum impurity decreases per feature'],
]

# --- Layout ---
COL_WIDTHS = [4.6, 3.6, 2.6, 6.0]
...
ax.text(TABLE_W / 2 + 1.0, caption_y,
        'n = training examples, d = features, depth = depth of fitted tree, '
        'nodes = total nodes including leaves.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out_path = 'D:/Projects/Medium/algorithms-in-python/01-supervised-learning/05-decision-trees/bigo_table.png'
