"""Generate a Big-O / cost summary table image for the NCF article."""
...
ROWS = [
    ...
    ['MLP score (i, j)',
     'O(k + H²)',
     'O(k + H²)',
     'dense hidden-layer matmuls'],
    ['NeuMF score (i, j)',
     'O(k + H²)',
     'O(k + H²)',
     'both towers, then fuse'],
    ['Training (one epoch)',
     'O(|Ω⁺|·(1+n_neg)·k)',
     'O((n + m) · k)',
     'negative sampling; sparse grads'],
    ['Embedding tables',
     'O((n + m) · k)',
     'O((n + m) · k)',
     'NeuMF: separate table per tower'],
    ['Top-N retrieval',
     'O(m · fwd)',
     'O(m)',
     'no ANN shortcut — score every item'],
]

COL_WIDTHS = [4.8, 4.8, 3.2, 6.4]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Neural Collaborative Filtering Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'n = users, m = items, k = embedding dim, H = MLP hidden width, '
        '|Ω⁺| = observed interactions, n_neg = negatives per positive.',
...
out = ('D:/Projects/Medium/algorithms-in-python/'
       ...
       '02-neural-collaborative-filtering/bigo_table.png')
