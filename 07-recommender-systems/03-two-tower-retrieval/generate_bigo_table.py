"""Generate a Big-O / cost summary table image for the Two-Tower article."""
...
ROWS = [
    ['Training, one batch',
     'O(B² · d + B · tower)',
     'O(B · d)',
     'in-batch softmax; batch is its own negatives'],
    ['Item indexing (offline)',
     'O(m · tower + m · log m)',
     'O(m · d)',
     'embed all items once, build ANN index'],
    ['Serving: user tower',
     'O(tower)',
     'O(d)',
    ...
    ['Serving: ANN lookup',
     'O(log m) approx.',
     'O(m · d)',
     'sub-linear; the whole point'],
    ['Serving total (two-tower)',
     'O(tower + log m)',
     'O(m · d)',
     'embed once + one index query'],
    ['Serving total (NeuMF)',
     'O(m · network)',
     'O(m · d)',
     'no ANN shortcut — scores every item'],
]

COL_WIDTHS = [5.0, 4.2, 2.8, 5.8]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Two-Tower Retrieval Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'B = batch size, d = embedding dim, m = catalogue size, '
        '"tower"/"network" = one forward pass cost.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out = ('D:/Projects/Medium/algorithms-in-python/'
       '07-recommender-systems/03-two-tower-retrieval/bigo_table.png')
