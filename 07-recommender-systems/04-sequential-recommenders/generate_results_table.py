"""Generate a results comparison table image for the Sequential Recommenders article."""
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Sequential recommendation: leave-one-out, 1 + 99 negatives',
...
for r, row in enumerate(ROWS):
    ...
    for i in (1, 2):
        # Highlight SASRec (winning) row in green
        color = '#059669' if r == 2 else (NOTE_COLOR if r == 0 else TEXT_COLOR)
        fw = 'bold' if r == 2 else 'normal'
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        '1000 users × 500 items, order-dependent sessions. '
        'Self-attention beats the bigram recency heuristic and popularity.',
        fontsize=9.5, ha='center', va='center',
        color=NOTE_COLOR, fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out = ('D:/Projects/Medium/algorithms-in-python/'
       '07-recommender-systems/04-sequential-recommenders/results_table.png')
