"""Generate a Big-O / cost summary table image for the Sequential Recommenders article."""
...
ROWS = [
    ['Self-attention, one sequence',
     'O(L² · d)',
     'O(L²)',
     'every position attends to earlier ones'],
    ['Feed-forward block',
     'O(L · d²)',
     'O(L · d)',
    ...
    ['Training (one epoch)',
     'O(n · L² · d)',
     'O(L² + m · d)',
     'parallel over positions and sequences'],
    ['Serving: encode history',
     'O(L² · d)',
     'O(L · d)',
     'one forward pass; cap L at ~50–200'],
    ['Serving: score catalogue',
     'O(m · d)  or  O(log m)',
     'O(m · d)',
     'dot product per item — or ANN index'],
    ['vs set-based model',
     '+O(L²) factor',
     'O(m · d)',
     'the price of modelling order'],
]

COL_WIDTHS = [5.4, 4.2, 3.0, 5.4]
...
ax.text(TOTAL_W / 2 + 1.0, TABLE_H + 1.3,
        'Sequential Recommender Complexity',
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'L = sequence length (capped), d = embedding dim, '
        'n = number of sequences, m = catalogue size.',
        fontsize=10, ha='center', va='center',
        color='#6B7280', fontfamily='sans-serif', fontstyle='italic')

plt.tight_layout()
out = ('D:/Projects/Medium/algorithms-in-python/'
       '07-recommender-systems/04-sequential-recommenders/bigo_table.png')
