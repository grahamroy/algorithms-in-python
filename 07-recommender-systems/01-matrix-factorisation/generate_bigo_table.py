"""Generate a Big-O / cost summary table image for the MF article."""
...
ROWS = [
    ['ALS, one iteration',
     'O((n + m) · k³ + |Ω| · k²)',
     'O((n + m) · k)',
     'closed-form ridge per entity'],
    ['SGD, one epoch',
     'O(|Ω| · k)',
     'O((n + m) · k)',
     'one update per observed rating'],
    ['Full training (ALS)',
     'O(I · ALS iter)',
     'O((n + m) · k)',
     'I = 10–20 typical'],
    ['Predict one (i, j)',
     'O(k)',
     'O(k)',
     'single dot product; sub-microsecond'],
    ['Top-N recommendation',
     'O(m · k + m · log N)',
     'O(m)',
     'score all items, partial sort'],
    ['Implicit-feedback ALS',
     'O((n + m) · k³ + |Ω| · k²)',
     'O((n + m) · k)',
     'Hu-Koren-Volinsky; same shape'],
]

COL_WIDTHS = [5.4, 4.6, 3.0, 5.4]
...
ax.text(TOTAL_W / 2 + 1.0, Y0 - 0.45,
        'n = users, m = items, k = latent dim, |Ω| = observed '
        'ratings, I = ALS iterations, N = recommendations.',
...
out = ('D:/Projects/Medium/algorithms-in-python/'
       ...
       '01-matrix-factorisation/bigo_table.png')
